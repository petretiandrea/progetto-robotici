-- utilities.lua - A robust collection class based on Laravel collections
--
-- @module utilities.lua
-- @alias Collection
...
function Collection:all() -- ATTENTION: "all" right after "collect" returns the same input collection
    return self.table
end

--- Adds an item to the end of a collection. ATTENTION: this method modifies the original collection.
function Collection:append(value)
    self.table[#self.table + 1] = value
    return self
end

--- Returns the average value of a list or given key
---@overload fun(): number
---@overload fun(key: any): number
...
function Collection:chunk(count)
    local chunks, chunksSize = self:new(), 0
    local currentChunk, currentChunkSize = {}, 0

    if count <= 0 then return Collection:new({ {} }) end

    for _, value in pairs(self.table) do
        currentChunk[currentChunkSize+1] = value
        currentChunkSize = currentChunkSize + 1
        if currentChunkSize == count then
            chunks.table[chunksSize + 1] = currentChunk
...
---@overload fun(containValue: any): boolean
---@overload fun(containValue: any, recursive: boolean): boolean
...
function Collection:forEachValue(callback)
    for _, value in ipairs(self.table) do
        if callback(value) == false then
            break
        end
    end
    return self
...
--- Filters the collection using the given callback, keeping only items that pass a truth test
function Collection:filterByValue(callback)
    local filtered = self:new()
    for key, value in pairs(self.table) do
        local response = false
        if callback then
...
---@overload fun(): any
---@overload fun(callback: fun(key, value): boolean): any
...
--- Returns the first element in the collection, or that passes a truth test
---@overload fun(): any
---@overload fun(callback: fun(value): boolean): any
function Collection:firstValue(callback)
    for _, value in pairs(self.table) do
        if callback then
...
---@overload fun(): table
---@overload fun(depth: number): table
---@overload fun(depth: number, tbl: table): table
---@overload fun(depth: number, tbl: table, currentDepth: number): table
...
--- Determines if a given value exists in the collection
function Collection:hasValue(element)
    for _, value in pairs(self.table) do
...
--- Determines if a given key-value pair exists in the collection
function Collection:exists(callback)
    return self:filter(callback):isNotEmpty()
end
...
--- Inserts a value at a given numeric index. ATTENTION: this method modifies the original collection.
...
--- Maps only the values of the table, keeping the keys intact
function Collection:mapValues(callback)
    local remapped = self:new()
    for key, value in pairs(self.table) do
        remapped:set(key, callback(value))
    end
    return remapped
end

--- Same as map followed by flatten
function Collection:flatMap(callback)
    return self:map(callback):flatten()
end

--- Same as mapValues followed by flatten
function Collection:flatMapValues(callback)
    return self:mapValues(callback):flatten()
end
...
---@overload fun(default: number): number
---@overload fun(default: number, maxKey: any): number
function Collection:max(default, maxKey)
    local max
    for _, value in pairs(self.table) do
        if maxKey then
    ...
    if(max == nil) then return default end
    return max
end
...
---@overload fun(): number|nil
---@overload fun(default: number): number
---@overload fun(default: number, minKey: any): number
function Collection:min(default, minKey)
    local min
    for key, value in pairs(self.table) do
        if minKey then
    ...
    if(min == nil) then return default end
    return min
end
...
--- Sets the given key and value in the collection. ATTENTION: this method modifies the original collection.
...
---@overload fun(count: number): table
---@overload fun(count: number, rep: boolean): table
function Collection:random(count, rep)
    ...
    for i = 1, count do
        if #all > 0 then
            local randomElement
            if rep then
                randomElement = all[math_random(#all)]
            else
                randomElement = table.remove(all, math_random(#all))
            end
...
--- Folds the collection to a single value from the left, passing the result of each iteration into the next
function Collection:foldLeft(callback, default)
    local carry = default
    for key, value in pairs(self.table) do
...
local function checkTableIncompatibility(table, functionName) -- assumption: a table with a value at index 1 is an array
    if (type(table) == "table" and table[1] == nil) then
        error(functionName .. " currently only works with arrays, not tables!")
    end
end

--- Folds the collection to a single value from the right, passing the result of each iteration into the next
function Collection:foldRight(callback, default)
    checkTableIncompatibility(self.table, "foldRight")
    local carry = default
    for index = #self.table, 1, -1 do
        carry = callback(carry, self.table[index])
    end
    return carry
end

--- Like foldLeft but it uses the first element of the collection as the 'default' value
---@return any
function Collection:reduce(callback) -- equivalent in scala: list.tail.foldLeft(list.head)(_)
    return self:tail():foldLeft(callback, self:head())
end
...
--- Returns two slices of the collection cut at the given index, without modifying the collection
function Collection:splitAt(index)
    result = self:new()
    result:push(self:take(index):all())
    result:push(self:slice(index):all())
    return result
end

--- Returns the whole collection except the first element
function Collection:tail()
    return self:slice(1)
end
...
local function copy_table_no_false_keys(table, keyList)
...
--- Returns all of the unique items in the collection (for example {1, 2, 2, 3} becomes {1, 3})
---@overload fun(): table
---@overload fun(callback: fun(key, value): any): table
function Collection:unique(callback)
    ...
    return copy_table_no_false_keys(self, keyList)
end

--- Removes the duplicates from the collection (for example {1, 2, 2, 3} becomes {1, 2, 3})
---@overload fun(): table
---@overload fun(callback: fun(key, value): any): table
function Collection:distinct(callback)
    local keyList = {}
...
--- Zips the collection with the indexes of the same collection.
function Collection:zipWithIndex()
    local indexes = {}
    local current_index = 1
    for _, _ in pairs(self.table) do
        indexes[current_index] = current_index
        current_index = current_index + 1
    end
    return self:zip(indexes)
end
...
--- Alias for the Collection:merge() method
Collection.addAll = Collection.merge
...
--- Alias for the Collection:chunk() method.
Collection.grouped = Collection.chunk
...
--- Alias for the Collection:splice() method
Collection.replace = Collection.splice

--- Alias for the Collection:first() method
Collection.head = Collection.first
